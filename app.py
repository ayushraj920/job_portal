...
def update_details(id):
    if "admin_user" in session:
        ...
        if request.method == 'POST':
            ...
            res = cur2.execute("SELECT * FROM resume WHERE jobseeker_id = {}".format(id))
            if res > 0:
                # if the resume entry exists for that user, then update the resume filename for the entry
                cur2.execute("UPDATE resume SET filename = (%s) WHERE jobseeker_id = (%s)", (filename, id))
                mysql.connection.commit()
            else:
...
def blockorunblock_user(id):
    if "admin_user" in session:
        ...
        if res > 0:
            ...
            if usrid[0][0] == "1":
                cur.execute("UPDATE jobseeker SET status = {} WHERE jobseeker_id = {}".format("0",id))
                mysql.connection.commit()
            else:
                cur.execute("UPDATE jobseeker SET status = {} WHERE jobseeker_id = {}".format("1",id))
                mysql.connection.commit()
    ...
    if "admin_user" in session:
        if request.method == 'POST':
            ...
            if keyword and (not location):
                count_search = cur.execute("SELECT job.job_id, job.job_title, job.job_type, company.name, company.location, job.job_salary, job.job_description, job.job_id FROM job INNER JOIN company ON job.company_id = company.company_id WHERE (job.job_title LIKE '{}') OR (job.job_type LIKE '{}') OR (job.job_description LIKE '{}')  OR (company.name LIKE '{}')".format(keyword, keyword, keyword,keyword))
            ...
            # and company location by using LIKE clause and inner join on jobs and corresponding companies
                ...
                count_search = cur.execute("SELECT job.job_id, job.job_title, job.job_type, company.name, company.location, job.job_salary, job.job_description, job.job_id FROM job INNER JOIN company ON job.company_id = company.company_id WHERE ((job.job_title LIKE '{}') OR (job.job_type LIKE '{}') OR (job.job_description LIKE '{}') OR (company.name LIKE '{}')) AND (company.location LIKE '{}')".format(keyword, keyword, keyword,keyword, location))
            else:
                count_search = 0
...
def jobs():
    if "user" in session:
        if request.method == 'POST':
            ...
            # by using LIKE clause and inner join on jobs and corresponding companies 
            if keyword and (not location):
                count_search = cur.execute("SELECT job.job_title, job.job_type, company.name, company.location, job.job_salary, job.job_description, job.job_id FROM job INNER JOIN company ON job.company_id = company.company_id WHERE (job.job_title LIKE '{}') OR (job.job_type LIKE '{}') OR (job.job_description LIKE '{}') OR (company.name LIKE '{}')".format(keyword, keyword, keyword,keyword))
            # if only location is entered in the job search, then select those jobs where the keyword matches the company location
            # by using LIKE clause and inner join on jobs and corresponding companies
            elif location and (not keyword):
            ...
            # and company location by using LIKE clause and inner join on jobs and corresponding companies
                ...
                count_search = cur.execute("SELECT job.job_title, job.job_type, company.name, company.location, job.job_salary, job.job_description, job.job_id FROM job INNER JOIN company ON job.company_id = company.company_id WHERE ((job.job_title LIKE '{}') OR (job.job_type LIKE '{}') OR (job.job_description LIKE '{}')  OR (company.name LIKE '{}')) AND (company.location LIKE '{}')".format(keyword, keyword, keyword,keyword,location))
            else:
